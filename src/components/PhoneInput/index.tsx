...
const PhoneInput = ({
  ...
  return (
    <div>
      <p className="text-[10px] font-ligh text-red-500 w-full text-start break-words capitalize">
        {errors}
      </p>

      <div
        className={`relative first-letter:custom-select flex w-full h-full flex-col ${
      ...
      >
        <div className="flex w-full items-center">
          ...
          <button
            ...
            ref={thisDDRef}
          >
            <span className="text-[12px] text-left tracking-[0px]">
          ...
          <div
            className={`bg-white border flex text-secondary border-gray-300 ${
              !isReadOnly &&
              "focus-within:bg-[#F5FAFF] focus-within:border-[#FF6C02]"
            } h-[35px] text-[12px] rounded-sm w-full`}
          >
            <input
              readOnly={isReadOnly}
              disabled={isLoading}
              ...
              maxLength={12}
              className="test-input bg-transparent text-inherit flex-grow p-2 outline-none w-full"
            />
            {!isReadOnly && (
